...
import { Language, CourseLevel, ActivityType } from '@prisma/client';

@Injectable()
export class CronActivityService implements OnModuleInit {
  private readonly logger = new Logger(CronActivityService.name);
  ...
  @Cron(CronExpression.EVERY_12_HOURS, { disabled: false })
  async syncActivitiesWithMongo() {
    for (const language of this.languages) {
      ...
      const codeOrgActivities = await this.getActivitiesFromCollection(
        collectionName,
        'code_organization',
      );
      const multipleChoiceActivities = await this.getActivitiesFromCollection(
        collectionName,
        'multiple_choice',
      );
      const codeCompletionActivities = await this.getActivitiesFromCollection(
        collectionName,
        'code_completion_exercise',
      );
      this.logger.log(
        `Atividades encontradas para ${language}: ${codeOrgActivities.length} CodeOrganization, ${multipleChoiceActivities.length} MultipleChoice, ${codeCompletionActivities.length} CodeCompletion`,
      );
      ...
      await this.processActivities(
        multipleChoiceActivities,
        language,
        'multiple_choice',
      );
      await this.processActivities(
        codeCompletionActivities,
        language,
        'code_completion_exercise',
      );
    }
  ...
  private async getActivitiesFromCollection(
    collectionName: string,
    documentType: string,
  ) {
    const model = this.connection.collection(collectionName);
    const activities = await model
      .find({ template_name: documentType })
      .toArray(); // Assume que há um campo "type" que identifica o documento
    return activities;
  }
  ...
  private async processActivities(
    ...
    for (const activity of activities) {
      ...
      if (!course) {
        this.logger.warn(
          `Curso não encontrado para ${language} e nível ${level}`,
        );
        continue;
      }
      ...
      if (!existingActivity) {
        ...
        this.logger.log(
          `Atividade sincronizada para ${course.name}: ${activity.question}`,
        );
      } else {
